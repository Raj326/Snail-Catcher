...
void setup(){
  ...
  Serial.println();
  Serial.printf("localConnectAck.comm: %i", localConnectAck.comm);
  initState();
}
...
int since_Clk() { // Raj:: originalmente estava retornando um boleano
  ...
  //Serial.printf("valor do sinceClk: %i", (millis() - Clk)/1000);
  //Serial.println();
...
void CT01(){
  localConnectAck.comm = false;
}

void CT02(){
  static int execs = 0;
  Serial.printf("execs: %i",execs);
  Serial.println();
  if(execs == 5){
    localConnectAck.comm = true;
    Serial.println("localConnectAck.comm = true");
  }
  if(execs == 6){
    state = final_snail;  
  }
...
void CT04(){
  state = joint1;
  localCoordinatesFiware.comm = false;
}
...
void test_9(){
  ...
  if(counter == 2){
    localCoordinatesFiware.comm = true;
    int list[4] = {0,0,0,0};
    int *n = list;
    localCoordinatesFiware.neighs = n;
  }
  if(counter == 3){
    localCheckedSnailSvAck.comm = true;
    localCheckedSnailSvAck.hasSnail = true;  
  }
  if(counter == 4){
    updateAck.comm = true;
  }
