...
EVENT connectAck, localConnectAck, updateAck;
...
SENDURL sendURL;
Connection connection("ssid","password","192.168.1.80","1026");
String localImage;
...
int line = 1;
int column = 1;
int timeout = 3*baseTime; // Raj::original: unsoigned long timeout = 3
int maxAttempt = 3;
bool cfT = true;
bool caT = true;
bool csT = true;
...
void registerRead() {
  ...
  connectAck.comm = connection.connectedWifi();
  if(connection.connectedWifi()){
    debug(2,"connected!");  
  }

  coordinatesFiware.neighs = connection.coordinatesFiware();
  coordinatesFiware.comm = true;
  
  Serial.print("Neighbors:");
  for(int i = 0; i <= 3; i++){
    Serial.printf("%i", coordinatesFiware.neighs[i]);  
  }
  
  localConnectAck.comm = connectAck.comm;
  localCoordinatesFiware.comm = coordinatesFiware.comm;

  localCoordinatesFiware.comm = coordinatesFiware.comm;
  if (localCoordinatesFiware.comm) {
    localCoordinatesFiware.neighs = coordinatesFiware.neighs;
    Serial.print("localNeighbors: ");
    for(int i = 0; i <= 3; i++){
        Serial.printf("%i", localCoordinatesFiware.neighs[i]);
    }
  //  localCoordinatesFiware.neighs[0] = coordinatesFiware.neighs[0];
  //  localCoordinatesFiware.neighs[1] = coordinatesFiware.neighs[1];
  //  localCoordinatesFiware.neighs[2] = coordinatesFiware.neighs[2];
  //  localCoordinatesFiware.neighs[3] = coordinatesFiware.neighs[3];
  }

  //localGetImage.comm = getImage.comm;
  //if (localGetImage.comm) {
  //  localImage = getImage.image;
  //}

  checkedSnailSvAck.comm = verificaBotao();
  if(checkedSnailSvAck.comm){
    checkedSnailSvAck.hasSnail = verificaBotao();
  }
  
  localCheckedSnailSvAck.comm = checkedSnailSvAck.comm;
  localCheckedSnailSvAck.hasSnail = checkedSnailSvAck.hasSnail;
  dump();
}
...
void updateFiware_out_c(int x, int y, ST state) { // updateFiware!(|x,y,SNAIL_sne|) // jmi: rever isso
  ...
  updateAck.comm = connection.updateAck();
}
...
void dump(){
  ...
  Serial.printf("%i", csT);
  Serial.println();
  Serial.print("checkedSnailSvAck.comm: ");
  Serial.printf("%i", checkedSnailSvAck.comm);
  Serial.println();
  Serial.print("Clk: ");
  Serial.printf("%i", since_Clk());
  Serial.println();
  Serial.printf("count: %i", count);
  Serial.println();
  Serial.println();
}
